...
| Release bundle | Always `C:\Diamond\pdf reader\releases\plainpdf-play.aab` (currently 1.0.0, version code 4, built 2026-09-06, signed, 69 MB with all twenty packs inside; Play serves each phone only the base plus the packs it asks for). Made by `./scripts/release.sh`, which bumps the version code, builds, and moves the previous bundle into `releases\archive\` named by version and date. Version codes 1 and 2 (archived) went to internal testing on 2026-09-05; version code 3 (archived) was never uploaded, it still had the icon crash; version code 4 is the one to upload; the extra `plainpdf-1.0.0-code2-play.aab` file is a one-off from before the convention and can be deleted |
...
| Quality gates | 140 unit tests green, `lintFossDebug` clean, CI workflow in `.github/workflows/android.yml` |
| Next step | **Live on Google Play since 2026-09-10** (version 1.0.0, code 4, approved after four days in review). Source published the same day; plainpdf.app/source links to it. Next update, version code 5: the working-notification title fix already on `main`, plus whatever the first users report |
...
- Baseline profile through a macrobenchmark module (spec M7; startup speed).
...
- 2026-09-10 Approved and live on Google Play. Source code published at github.com/lazytitan30/plainpdf; the website's source page links to it.
